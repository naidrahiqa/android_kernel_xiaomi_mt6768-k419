...
static void aw87xxx_fw_load(const struct firmware *fw, void *context)
{
	...
	AW_DEV_LOGI(aw87xxx->dev, "enter, fw=%p", fw);

	if (!fw) {
/* Legacy binary loader by vrdons start */
		mutex_lock(&aw87xxx->reg_lock);
		...
		if (ret == 0) {
			...
			if (ret == 0) {
				...
				AW_DEV_LOGE(aw87xxx->dev, "aw87xxx_init_default_prof failed, ret=%d", ret);
			}
		} else {
			AW_DEV_LOGE(aw87xxx->dev, "aw87xxx_legacy_bin_load failed, ret=%d", ret);
		}
		mutex_unlock(&aw87xxx->reg_lock);

		aw87xxx_fw_load_retry(aw87xxx);
/* Legacy binary loader by vrdons end */
		return;
	}
				...
				struct device_node *dev_node)
{
	...
	if (ret < 0)
		ret = of_property_read_u32(dev_node, "pa-channel", &dev_index);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		if (gpio_is_valid(aw87xxx->aw_dev.rst_gpio)) {
			...
			if (ret < 0) {
				AW_DEV_LOGI(aw87xxx->dev, "reset gpio already requested (shared reset pin)");
			}
		}
	}
